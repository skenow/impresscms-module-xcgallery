...
function get_subcat_data_block($parent, $ident = '') {
	...
	if (($cat_count = $xoopsDB->getRowsNum($result)) > 0) {
		...
		foreach ($rowset as $subcat) {
			if ($pos > 0) {
				$CAT_LIST_BLOCK[] = array('cid' => $subcat['cid'], 'parent' => $parent, 'pos' => $pos++ , 'prev' => $prev_cid, 'cat_count' => $cat_count, 'name' => $ident . icms_core_DataFilter::htmlSpecialchars($subcat['name']));
				$CAT_LIST_BLOCK[$last_index]['next'] = $subcat['cid'];
			} else {
				$CAT_LIST_BLOCK[] = array('cid' => $subcat['cid'], 'parent' => $parent, 'pos' => $pos++ , 'cat_count' => $cat_count, 'name' => $ident . icms_core_DataFilter::htmlSpecialchars($subcat['name']));
			}
			$prev_cid = $subcat['cid'];
