...
function get_subcat_data($parent, $ident = '') {
	...
	if (($cat_count = icms::$xoopsDB->getRowsNum($result)) > 0) {
		...
		foreach ($rowset as $subcat) {
			if ($pos > 0) {
				$CAT_LIST[] = array('cid' => $subcat['cid'], 'parent' => $parent, 'pos' => $pos++ , 'prev' => $prev_cid, 'cat_count' => $cat_count, 'name' => $ident . icms_core_DataFilter::htmlSpecialchars($subcat['name']));
				$CAT_LIST[$last_index]['next'] = $subcat['cid'];
			} else {
				$CAT_LIST[] = array('cid' => $subcat['cid'], 'parent' => $parent, 'pos' => $pos++ , 'cat_count' => $cat_count, 'name' => $ident . icms_core_DataFilter::htmlSpecialchars($subcat['name']));
			}
			$prev_cid = $subcat['cid'];
...
switch ($op) {
	...
	case 'updatecat':
		...
		$name = trim($_POST['name']) ? icms_core_DataFilter::addSlashes($_POST['name']) : '&lt;???&gt;';
		$description = icms_core_DataFilter::addSlashes($_POST['description']);
