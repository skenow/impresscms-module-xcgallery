...
class smtp {
	 ...
	 */
	function auth() {
		if (is_resource($this->connection) and $this->send_data('AUTH LOGIN') and substr(trim($error = $this->get_data()), 0, 3) === '334' and $this->send_data(base64_encode($this->user)) and // Send username
		substr(trim($error = $this->get_data()), 0, 3) === '334' and $this->send_data(base64_encode($this->pass)) and // Send password
