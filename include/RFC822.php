...
class Mail_RFC822 {
	...
	/**
	 * Sets up the object.
	 * The address must either be set here or when
	 ...
	 * The address must either be set here
	...
	function _splitAddresses($address) {
		...
		if ($is_group) {
			...
			if (!$this->_splitCheck(explode(':', $string), ':')) return false;
		...
		$this->addresses[] = array('address' => trim($string), 'group' => $is_group);
	...
	function _hasUnclosedQuotes($string) {
		...
		for ($i = 0; $i < (count($string) - 1); $i++ )
			if (substr($string[$i], -1) == '\\') $string_cnt-- ;
	...
	function _hasUnclosedBracketsSub($string, &$num, $char) {
		...
		for ($i = 0; $i < count($parts); $i++ ) {
			if (substr($parts[$i], -1) == '\\' || $this->_hasUnclosedQuotes($parts[$i])) $num-- ;
			if (isset($parts[$i + 1])) $parts[$i + 1] = $parts[$i] . $char . $parts[$i + 1];
		}
	...
	function _validatePhrase($phrase) {
		...
		for ($i = 0; $i < count($phrase_parts); $i++ ) {
			...
			if (substr($phrase_parts[$i], 0, 1) == '"') {
				if (!$this->_validateQuotedString($phrase_parts[$i])) return false;
				continue;
			}
	...
	function validateMailbox(&$mailbox) {
		...
		if (substr($mailbox, -1) == '>' && substr($mailbox, 0, 1) != '<') {
			...
			if ($this->_validatePhrase($phrase) === false || ($route_addr = $this->_validateRouteAddr($route_addr)) === false) return false;
			...
			if (($addr_spec = $this->_validateAddrSpec($addr_spec)) === false) return false;
		}
	...
	function _validateDomain($domain) {
		...
		for ($i = 0; $i < count($sub_domains); $i++ ) {
			if (!$this->_validateSubdomain(trim($sub_domains[$i]))) return false;
		}
	 ...
	 * in the given string.
	 * This is APPROXIMATE as it only splits
	 ...
	 * of the class.
	 * It simply validates whether an email is of
